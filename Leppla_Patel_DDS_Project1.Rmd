...
```{r import, echo=FALSE}
Beers = read.csv("C:/Users/allep/OneDrive/Documents/MDS-6306-Doing-Data-Science/Unit 8 and 9 Case Study 1/Beers.csv",header=T)
Breweries = read.csv("C:/Users/allep/OneDrive/Documents/MDS-6306-Doing-Data-Science/Unit 8 and 9 Case Study 1/Breweries.csv",header=T)
states = read.csv("~/untitled folder/cs1/FinalCS1/SMU-Doing-Data-Science-Project1/states.csv")
beer_reviews = read.csv("~/Downloads/beer_reviews.csv")
...
Do IPAs include:
- Double / Imperial IPAs (high alcohol) 
- India Style Session Ales (low alcohol)? 
- American, American White, Belgian, and English IPAs?
...
There is a trade-off between Sensitivity and Specificity with the kNN model.  k=5 gives  higher sensitivity (classifies fewer Ales as IPAs), but k=11 gives higher specificity (classifies fewer IPAs as Ales).         
...
- Unexplored variables: City, Brewery, Ounces, Style 
- Are big beers big on alcohol?  Bombers

```{r}

summary(Beer$Ounces) 

#Bigger beer volume > 16 oz appear to have higher alcohol content
library(plotly)
p = Beer %>% ggplot(aes(Ounces,ABV1,color=Style)) + geom_jitter() + theme(legend.position = "none") 
ggplotly(p)

Beer %>% filter(ABV1>0.099) %>% ggplot(aes(Style,fill=Style)) + geom_bar() + theme(axis.text.x = element_blank()) + ylab("# of 10%+ ABV Beers") + coord_cartesian(ylim=c(0,11))

#Based on state-by-state regulations?
Beer %>% filter(ABV1==0.099) %>% ggplot(aes(State,fill=State)) + geom_bar() + ylab("# of 9.9% ABV Beers") + coord_cartesian(ylim=c(0,8)) + theme(legend.position = "none")

Beer %>% filter(ABV1>0.098) %>% ggplot(aes(State,fill=State)) + geom_bar() + ylab("# of 9.9%+ ABV Beers") + coord_cartesian(ylim=c(0,8)) + theme(legend.position = "none") 

#CO, IN, NY, KY have beers > 9.9% ABV
...
- What makes it a Pale Ale vs. IPA anyway?

IPAs are generally (but not always) hoppier and boozier than Pale Ales.  American Double / Imperial IPAs are pushing this trend to new limits 
